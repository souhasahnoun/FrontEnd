...
@Component({
  selector: 'app-lignefactureavoirc',
  templateUrl: './lignefactureavoirc.component.html',
  styleUrls: ['./lignefactureavoirc.component.css']
})
export class LignefactureavoircComponent implements OnInit {
 ...
 nom_prod:string;
  ...
  fetchDefaultSupportedTvas() {
    ...
    modifierFactureAvoirClts(){
      // fonction modifier
      if (this.modif){
        this.documentsService.modifier(this.document).subscribe(data => {
    
          this.message='facture avoir modifiée avec success';
          console.log(data);
        }, error =>{
          console.log(error);
          this.message='Erreur';
